...
| 27 | Răspunsuri libere cu rubrică, pentru materiile fără execuție | Terminat | Schema avea deja `exercitiu.tip = 'liber'` și `exercitiu.rubrica` din migrarea 0000 — nicio migrare nouă. `lib/generare/curs.ts`: când drumul „scrie funcția" (Pyodide) nu dă ceva utilizabil, se încearcă acum un fallback cu răspuns liber (`genereazaExercitiulLiber`) — enunț + 2-4 puncte de rubrică, în loc să se sară conceptul direct. Evaluarea, în `lib/exercitii/evalueaza-liber.ts`: prompt izolat care vede *doar* rubrica și răspunsul, niciodată conversația (`PLAN.md` §12) — singurul loc din aplicație unde verdictul trece prin model (regula 1 vorbește despre exerciții cu execuție). Fără model, răspunsul tot se salvează, cu XP-ul garantat al unei încercări, dar verdict „neevaluat" — onest, nu ascuns (regula 5). `lib/date/incercari.ts` s-a generalizat (`scrieIncercare` ia acum `{verdict, trecute, total}`, nu tipul `Raport` legat de motorul de cod), ca să servească ambele drumuri. UI nou în `/lectie/`: `<textarea>` în loc de editor, `componente/raport-rubrica.tsx` în loc de raportul de cazuri — rubrica nu se arată înainte de răspuns, ca să nu devină „completează golurile". Testat cap-coadă: am regenerat „Materialul tău" cu graful de concepte existent (14 concepte, multe halucinate din testarea pasului 19) — au ieșit 10 lecții noi, față de 1 din 14 înainte de fallback. Am jucat un exercițiu liber de două ori: o dată cu modelul nepornit (verdict „neevaluat", răspunsul păstrat, +5 XP, ecranul explică de ce fără să blocheze nimic), o dată cu modelul pornit (evaluare reală: 0 din 4 puncte atinse pe rubrica generată, explicația predefinită arătată, XP total urcat de la 50 la 55). N-am ajuns să văd live un verdict „corect" sau „parțial" — conceptele astea sunt ele însele de calitate slabă (moștenire din pasul 19), deci rubricile ies la fel; e limita acceptată a unui model mic (`PLAN.md` §12), nu un bug de reparat aici. |
