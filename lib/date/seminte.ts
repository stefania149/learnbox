/**
 * Trei exerciții scrise de mână, cât să existe ce rula la pasul 4 — câte unul
 * din fiecare tip din `PLAN.md` §7, în ordinea în care cer efort.
 *
 * Capitolul adevărat de Python se scrie la pasul 8; acestea se seamănă o
 * singură dată, la prima deschidere, și rămân în baza locală.
 */
...
/**
 * Seamănă cursul demonstrativ dacă lipsește și întoarce exercițiile lui.
 * Idempotentă: două file deschise odată nu trebuie să scrie de două ori.
 */
export async function exercitiileDemo(): Promise<Exercitiu[]> {
  ...
  const capitole = await baza
    .select()
    .from(capitol)
    .where(eq(capitol.materieId, id));

  if (capitole.length === 0) {
    const [cap] = await baza
      .insert(capitol)
      .values({ materieId: id, nume: "Primele funcții", ordine: 1 })
      .returning();
    ...
    await baza
      .insert(exercitiu)
      .values(EXERCITII.map((e) => ({ ...e, nivelId: niv.id })));
  }
  ...
  return baza
    .select()
    .from(exercitiu)
    .where(eq(exercitiu.nivelId, niveluri[0].nivel.id))
    .orderBy(asc(exercitiu.id));
}

/** Materia sub care se adună XP-ul. */
export async function materiaDemo(): Promise<number> {
  const { baza } = await deschideBaza();
